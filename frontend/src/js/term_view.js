...
export function TermView() {
  ...
  this.trackpadSmoothScroll = true;
  this.trackpadScrollSpeed = 2;
  //this.highlightFG = 7;
  ...
  // Default to non-selectable (menus); article reading (pageState 3) re-enables it.
  mainDisplay.style.webkitUserSelect = 'none';
...
TermView.prototype = {
  ...
  redraw: function(force) {
    ...
    if (changedLineHtmlStrs.length > 0) {
      // Only allow text selection while reading an article (pageState 3).
      // On menus / lists, disable selection so navigating doesn't accidentally
      // (or automatically) highlight text. Must be set on .main itself (inline)
      // to override its CSS `-webkit-user-select: text`.
      var _selectable = (this.buf.pageState == 3) ? 'text' : 'none';
      this.mainDisplay.style.webkitUserSelect = _selectable;
      this.mainDisplay.style.userSelect = _selectable;
      ...
      if (this.useEasyReadingMode) {
        ...
        this.componentScreen = renderScreen(
          ...
          this.chh,
          /* enableLinkInlinePreview = 自動開圖: inline images below the link */
          this.enablePicPreview,
          /* enableLinkHoverPreview = OFF: no floating hover preview */
          false,
          this.mainDisplay
        )
  ...
  setHighlightedRow: function(row) {
    // Always show the hover indicator in mouse-browsing (the old on/off "底色"
    // option was removed); styling lives in CSS (.mb-hover).
