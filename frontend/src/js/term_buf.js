...
TermBuf.prototype = {
  ...
  notify: function(timer) {
    ...
    if (this.changed) { // content changed
      ...
      var prevPageStateForHL = this.pageState;
      this.setPageState();
      var pageStateChanged = (prevPageStateForHL !== this.pageState);
      if (this.useMouseBrowsing && pageStateChanged) {
        // Clear the hover highlight only when the page actually changes — not on
        // every minor update (e.g. the status line's clock), which would make
        // the hover indicator flicker / vanish.
        this.clearHighlight();
      }
      ...
      if (this.view) {
        ...
        if (pageStateChanged) {
          // New screen → scroll to top. Inline images ("自動開圖") can make an
          // article tall & scrolled; without this, returning to the short list
          // leaves the viewport scrolled past the content = a black flash.
          if (this.view.mainDisplay && !this.view.useEasyReadingMode) {
            this.view.mainDisplay.scrollTop = 0;
          }
          // Re-derive the hover bar from the mouse position so a transient
          // pageState flip doesn't leave it gone until the next mouse move.
          if (this.useMouseBrowsing && this.view.bbscore &&
              this.view.bbscore.refreshMouseHighlight) {
            this.view.bbscore.refreshMouseHighlight();
          }
        }
      }
